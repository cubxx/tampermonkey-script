...
const tm = (() => {
  ...
  const _ = {
    throw: Object.assign(
      ...
      (msg, ...e) => {
        log.error(msg, ...e);
        throw Error(msg, { cause: _.throw.cause });
      },
      { cause: 'tm:throw' },
    ),
    /**
    ...
    toggle(data, key, values) {
      ...
      return value == null
        ? _.throw('toggle value is nullish', index, values)
        : (data[key] = value);
    },
  };
  const log = (() => {
  ...
  const hack = {
    ...
    override(obj, key, convert) {
      ...
      if (!d.configurable) throw _.Err(`${key.toString()} is not configurable`);
      //@ts-ignore
    ...
    enableDevtool() {
      hack.override(Object, 'assign', ({ value }) => ({
        value(...e) {
          if (typeof e[0] === 'function' && _.hasOwn(e[1], 'isDevToolOpened')) {
    ...
    (selector, target = document) => target.querySelector(selector),
    {
      ...
      async wait(selector, options) {
        ...
        while (true) {
          ...
          if (res) return res;
          if (++count > opts.count) _.throw('wait timeout', selector);
        }
      },
    },
  );
  ...
  const tm = /** @type {const} */ ({
    ...
    async import(url) {
      ...
      if (!res.ok) _.throw('fetch error:', res);
      const type = res.headers.get('Content-Type');
      if (!type?.includes('javascript')) _.throw('not js', type);
      const code = await res.text();
      return new Function(code)();
    },
    /** @typedef {{ [key: string]: RouteMap | (() => void) }} RouteMap */
    /** Only idempotent handler @param {RouteMap} map */
    router(map) {
      const run =
        ...
        async (node, parts, route = '') => {
          ...
          if (typeof node === 'function') {
            if (_.hasOwn(tm.router, route)) return; // already handled
            log.info('route', location.host + route);
            tm.router[route] = node;
            try {
              await node();
            } catch (error) {
              if (Error.isError(error) && error.cause === _.throw.cause) return;
