...
const tm = (function () {
  ...
  const hack = {
    ...
    cancelTrustedTypes(name = 'tm-policy') {
  ...
  const $ = Object.assign(
    ...
    },
    {
       ...
       * @param {(
       *   this: HTMLElement,
       *   props: Record<P, Signal<string | null>[0]>,
       * ) => () => ReturnType<typeof lit.html>} setup
       * @param {ComponentOptions<P>} options
       * @param factory
       */
      Component(setup, options) {
        const propDefs = options.props ?? [];
        const getters = /** @type {Record<P, Signal<string | null>[0]>} */ ({}),
          setters = /** @type {Record<P, Signal<string | null>[1]>} */ ({});
          ...
          class extends HTMLElement {
            ...
            constructor() {
              ...
              propDefs.forEach((prop) => {
                const [getter, setter] = signal.createSignal(
                  this.getAttribute(prop),
                );
                getters[prop] = getter;
                setters[prop] = setter;
              });
              const toTemplate = setup.call(this, getters);
            ...
            connectedCallback() {
              options.onMounted?.forEach((cb) => cb());
            }
            disconnectedCallback() {
              options.onUnmounted?.forEach((cb) => cb());
            }
            attributeChangedCallback(name, oldValue, newValue) {
              setters[name](newValue);
            }
          },
        );
      },
  ...
  const tm = /** @type {const} */ ({
      ...
      window.customElements.define(
        ...
        class extends HTMLElement {
          ...
          constructor() {
            ...
            const root = this.attachShadow({ mode: 'open' });
            const to_template = setup(getters);
            signal.createEffect(() => {
              lit.render(to_template(), root);
            });
          }
          attributeChangedCallback(name, oldValue, newValue) {
    ...
    /**
     * @param {string} msg @param {string[]} selectors
...
tm.matchURL(
  ...
  ['translate.google.com', () => tm.hack.cancelTrustedTypes()],
  ['fanyi.youdao.com', () => tm.hack.enableDevtool('chunk-vendors')],
  ['xiaoeknow.com', () => tm.hack.disableInfDebugger()],
  [
    'copilot.microsoft.com',
    () => tm.hack.cancelTrustedTypes('@centro/hvc-loader'),
  ],
);
